...
import br.edu.puccampinas.pi3_es_2024_time_25.databinding.ActivityDuasPessoasAlocacao1Binding
...
class DuasPessoasAlocacao1Activity : AppCompatActivity() {
    private lateinit var binding: ActivityDuasPessoasAlocacao1Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityDuasPessoasAlocacao1Binding.inflate(layoutInflater)
        ...
        binding.nextPersonButton.setOnClickListener {
            // Verifica se há uma segunda imagem disponível
            if (currentImageIndex == 0) {
                // Se sim, envia o usuário para a próxima atividade para exibir a segunda imagem
                val intent = Intent(this, DuasPessoasAlocacao2Activity::class.java)
                intent.putExtra("documentId", documentId)
                startActivity(intent)
            } else {
                Toast.makeText(this, "Segunda imagem não disponível", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        firestore.collection("rents").document(documentId)
            ...
            .addOnSuccessListener { documentSnapshot ->
                if (documentSnapshot.exists()) {
                    ...
                    val imageNames = data?.get("images") as? ArrayList<*>
                    if (imageNames != null && imageNames.isNotEmpty()) {
                        displayImage(imageNames[currentImageIndex].toString())
                    } else {
